...
void main() {
  group('BusinessDetailsPage', () {
    ...
    testWidgets(
        'filling business name with category selected enables Create Business button',
        (tester) async {
      tester.view.physicalSize = const Size(375, 812);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      final businessNameFinder =
          find.widgetWithText(TextField, 'e.g. Abel Photography');
      await tester.enterText(businessNameFinder, 'Abel Photography');
      await tester.pump();

      final buttonFinder =
          find.widgetWithText(ElevatedButton, 'Create Business');
